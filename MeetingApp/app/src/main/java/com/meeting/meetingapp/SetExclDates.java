...
public class SetExclDates extends AppCompatActivity {
    private Button btn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.set_excl_dates);
        setupReferences();
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                            ...
                            if (userEmail.equals(snapEmail)){
                                ...
                                // set to true if clash of pref found
                                boolean clashOfPrefs = false;
                                List<List<String>> prefTimes = (List<List<String>>) dataSnapshot.child(snapshot.getKey()).child("prefTimes").getValue();
                                List<List<String>> exclTimes = dayList;
                                for (int day = 0; day < prefTimes.size(); day++){
                                    for (int timeSlot = 0; timeSlot < prefTimes.get(day).size(); timeSlot++){
                                        if(prefTimes.get(day).get(timeSlot).equals(exclTimes.get(day).get(timeSlot))){
                                            if (!prefTimes.get(day).get(timeSlot).equals("")){
                                                clashOfPrefs = true;
                                            }
                                ...
                                if (clashOfPrefs){
                                    Toast.makeText(SetExclDates.this, "You have set a preference and exclusion on the same day, please go back.", Toast.LENGTH_SHORT).show();
                                }
                                else{
                                    database.getReference().child("users").child(snapshot.getKey()).child("exclTimes").setValue(dayList);
                                    Intent intent = new Intent(SetExclDates.this, MainActivity.class);
    ...
    private void setupReferences() {
        ...
        lvCategory = findViewById(R.id.exclDateList);
